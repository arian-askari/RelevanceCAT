# -*- coding: utf-8 -*-
"""train_all-MiniLM-L12-v2.1-bm25added.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ctPhtgAtGFPlzByQnazcg0tdJbQ9cg6C

# Installiation and import libraries
"""
...
scores_path = injection_scores_path + "1_bm25_scores_train_triples_small.json"
global_min_bm25 = 0
global_max_bm25 = 50
num_epochs = 5
pos_neg_ratio = 4
...
queries_path = base_path + "queries.train.tsv"
...
model_max_length = 230 + 3 # 3:[cls]query[sep]doc[sep]. Because we do not have injecting into the input, we do not consider 3 token for v2.1! 3 extra tokens are needed in injection because bm25 score: normally takes two tokens, and onre more sep bm25 score [sep]
...
for qid in tqdm.tqdm(scores.keys(), desc = "reading scores...{}".format(scores_path)):
  for did, score in scores[qid].items():
    ...
    normalized_score = float(normalized_score * 10) # do not multiply to 100 so keep the number in low range similar to other teachers' logits
...
teacher_logits_filepath = os.path.join(base_path, 'bert_cat_ensemble_msmarcopassage_train_scores_ids.tsv')
train_samples = []
...
def read_train_triples_logits_and_bm25added(f_path, queries, corpus, max_instances = 0 , pos_neg_ratio = 4, dev_qids = set(), scores = {}):
  samples = []
  with open(f_path) as fIn:
    for line in fIn:
        ...
        if qid in dev_qids: #Skip queries in our dev dataset. The logits file contains the train set as well so we shold skip the dev queries.
            continue
        pos_score = ((float(pos_score)*3) + scores[qid][pid1])/4 # adding bm25 score to ensemble mean :)
        neg_score = ((float(neg_score)*3) + scores[qid][pid2])/4 # adding bm25 score to ensemble mean :)
...
train_samples = read_train_triples_logits_and_bm25added(triples_train_path, queries, corpus, max_train_samples, pos_neg_ratio, dev_qids = dev_samples.keys(), scores = scores)
...
model.fit(train_dataloader=train_dataloader,
          ...
          use_amp=True,
          optimizer_params = {'lr': 2e-5}, # sentence bert config! however,  Hostätter et al. has used 7e-6
